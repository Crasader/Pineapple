...
bool InputController::touchesBeganCB(std::vector<Touch*> touches, timestamp_t current) {
    if (touches.size() == 1) {
        ...
        // beginning potential grow/shrink
        _id1 = touches[0]->getID();
        _touch1 = touches[0]->getLocation();
        _id2 = touches[1]->getID();
        _touch2 = touches[0]->getLocation();
        _previousDelta = _touch1.distance(_touch2);
        return true;
    } else {
        // ignoring more than 2 touches
        return true;
    }
}
...
void InputController::touchesMovedCB(std::vector<Touch*> touches, timestamp_t current) {
    ...
    if (touches.size() >= 1) {
        ...
        if (_id1 != -1 && _id2 != -1) {
            checkPinchSpread();
            // TODO use the above output to set _keyGrow and _keyShrink
        }
        _previousDelta = _touch1.distance(_touch2);
    }
}
